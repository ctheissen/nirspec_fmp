...
def pixelWaveShift(data, model, start_pixel,window_width=40,delta_wave_range=10,model2=None,test=False,testname=None,gaussian=True,counter=None,**kwargs):
	"""
	...
	if test is True:
		...
		ax3.plot([best_shift,best_shift],[float(np.min(xcorr_list)),float(np.max(xcorr_list))],'k:',label=r"best wavelength shift:{}($\displaystyle \AA $)".format(round(best_shift,5)))
...
def wavelengthSolutionFit(data,model,**kwargs):
	...
	# getting the parameters of initial wavelength solution 
		...
		popt, pcov = curve_fit(lambda pixel, wfit0, wfit1, wfit2, wfit3, wfit4, wfit5, c3, c4: \
			nsp.waveSolution(pixel, wfit0, wfit1, wfit2, wfit3, wfit4, wfit5, c3, c4,order=order), width_range_center, best_shift_list,p0)
	...
	if save is True:
		...
		with fits.open(data_path) as hdulist:
			...
			hdulist[0].header['FITSTD']   = np.std(nsp.waveSolution(width_range_center2,popt2[0]\
				,popt2[1],popt2[2],popt2[3],popt2[4],popt2[5],popt2[6],popt2[7],order=order) - best_shift_array2)
			...
			hdulist[0].header['STD']    = str(np.std(residual2)/np.average(new_wave_sol)*299792.458) + 'km/s'
			hdulist[0].data             = nsp.waveSolution(np.arange(1024)+1,wfit0,wfit1,wfit2,wfit3,wfit4,wfit5,c3,c4,order=order)
